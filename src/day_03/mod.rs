...
pub fn day_03() {
    ...
    let solution_a = part_a(&rucksacks);
    println!("\t- Solution A is : {}", solution_a);

    let solution_b = part_b(&rucksacks);
...
fn part_a(rucksacks: &[Vec<char>]) -> i32 {
    rucksacks
        .iter()
...
fn get_char_priority(ch: &char) -> i32 {
    let value = *ch as i32;
    if value >= 97 { value - 96 } else { value - 38 }
}


fn part_b(rucksacks: &[Vec<char>]) -> i32 {
    ...
    for rucksacks_chunk in hashset_rucksacks.chunks(3) {
        let first_intersection: HashSet<_> = rucksacks_chunk[0].intersection(&rucksacks_chunk[1]).map(|c| c.to_owned()).collect();
        let total_intersection = first_intersection.intersection(&rucksacks_chunk[2]);
        item_types.push(get_char_priority(total_intersection.into_iter().next().unwrap()))
    }
    item_types.iter().sum()
}
